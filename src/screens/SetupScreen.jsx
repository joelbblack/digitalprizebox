...
export default function SetupScreen() {
  ...
  const finish = async () => {
    if (!profile && user) {
      await supabase.from("users").update({ signup_fee_paid: true })
        .eq("auth_id", user.id);
    } else if (profile) {
      await supabase.from("users").update({ signup_fee_paid: true })
        .eq("id", profile.id);
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          fontSize: 13, color: "#94A3B8", marginBottom: 24, lineHeight: 1.7,
        }}>
          You are signed up as a{" "}
          <strong style={{ color: "#F1F5F9" }}>{role}</strong>.
          Tap below to go to your dashboard.
        </div>
        <button onClick={finish} style={{
          width: "100%", background: "#6366F1", border: "none",
          color: "white", borderRadius: 10, padding: "12px 0",
